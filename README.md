...
- Windows 10/11 (64-bit)에서 사용 가능합니다.
- Linux는 개발/빌드 환경을 지원하며, 배포 경험은 Windows보다 덜 검증되었습니다.
...
- Node.js >= 20
- Rust (최신 stable)
- Windows 10/11 (64-bit) 또는 Linux (Ubuntu 24.04 기준 검증)
- Windows에서 빌드하려면 Microsoft Visual Studio C++ Build Tools 또는 Visual Studio Community
  - 설치 시 **Desktop development with C++** 워크로드 포함

### Linux / WSL (Ubuntu 24.04) 추가 준비

Tauri v2 데스크톱 앱을 Linux에서 실행하려면 시스템 패키지가 필요합니다.
...
2. **Rust 확인**
   ...
   ```
3. **MSVC 빌드 도구 확인**
   - Tauri Windows 개발에는 Microsoft C++ Build Tools가 필요합니다.
   - 설치가 안 되어 있으면 Tauri 앱 빌드/실행 단계에서 컴파일 오류가 발생할 수 있습니다.
4. **프로젝트 준비**
   ```bash
   npm install
   npm run typecheck
...
- 소스 편집은 Windows/WSL 어느 쪽에서도 가능하지만, 같은 체크아웃에서 `node_modules`, `dist`, `src-tauri/target` 빌드는 섞어 쓰지 않는 것을 권장합니다.
- Windows를 주 개발 환경으로 쓸 때는 Windows에서 `npm install`, `npm run dev`를 실행하세요.
- Linux/WSL을 주 개발 환경으로 쓸 때는 Linux 쪽에서 의존성을 다시 설치하고 실행하세요.
- WSL에서 `npm run dev`로 GUI 앱을 띄우려면 WSLg 같은 GUI 세션이 필요합니다.
- 이 저장소는 `.gitattributes`와 `.editorconfig`로 소스 파일 줄바꿈을 LF 기준으로 고정합니다.
...
- Windows 우선 디자인(Windows 11 스타일 제목 표시줄)
- Linux 개발/빌드는 가능하지만 일부 배포 시나리오는 추가 검증이 필요할 수 있음
