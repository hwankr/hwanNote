...
- `v0.9.12`
  - 불완전한 디렉터리/파일 스캔에서 인덱스와 캘린더 연결을 보존
  - 손상 인덱스 원본을 보호 백업하고 복구 전 모든 쓰기 차단
  - 오류 경로와 원인을 표시하는 노트 라이브러리 복구 패널 및 재시도 제공
  - 완전한 스캔에서만 실제 삭제된 노트와 orphan 연결 정리
- 최근 작업 기준
  - 라이브러리 메모 자동 저장 복구
